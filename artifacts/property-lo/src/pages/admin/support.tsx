import { useGetAdminSupportChats } from '@workspace/api-client-react';
import { Skeleton } from '@/components/ui/skeleton';
import { LifeBuoy, ChevronRight, User as UserIcon } from 'lucide-react';
import { Link } from 'wouter';
import { Badge } from '@/components/ui/badge';
import { formatDistanceToNow } from 'date-fns';

export default function AdminSupport() {
  const { data: chats, isLoading } = useGetAdminSupportChats({ query: { refetchInterval: 10000 } });
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-16 text-center">
            ...
            <h3 className="text-xl font-medium text-foreground mb-2">No active support tickets</h3>
          ...
          <div className="divide-y divide-border">
            {chats.map(chat => (
              <Link key={chat.id} href={`/admin/support/${chat.id}`}>
